...
One way to start developing a custom translator is to create a project using the {{ book.productnameFull }} archetype template. When the project is created from the template, it will contain the essential classes (i.e., ExecutionFactory) and resources for you to begin adding your custom logic. Additionally, the maven dependencies are defined in the pom.xml so that you can begin compiling the classes.
...
----
  ...
  -Dtranslator-type -  (user defined) the translator type that's used by {{ book.productnameFull }} when mapping the physical source to the translator to use 
