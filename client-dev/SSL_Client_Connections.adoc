...
There are two different sets of properties that a client can configure to enable 1-way or 2-way SSL. See also the https://docs.jboss.org/author/display/TEIID/Teiid+Server+Transport+Security[Teiid Server Transport Security] chapter if you are responsible for configuring the server as well.
