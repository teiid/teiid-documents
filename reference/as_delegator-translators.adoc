...
You could use the delegator translator to turn on the `ORDER BY` support without actually writing any code. 
...
The following example shows how to override the "hive" translator and turn off the `ORDER BY` support.
...
The preceding example shows how you might modify the default ORDER BY support for the Hive translator. 
