...
* **odata.war** - {{ book.productnameFull }} support OData via odata.war. For more information, see _OData support_ in the link:../client-dev/OData_Support.adoc[Client Developer's Guide]. 
...
For more information, see {% if book.targetDVProd %}xref:federated-planning{% else %}link:as_federated-planning.adoc{% endif %}[Federated planning].
...
Translator:: {{ book.productnameFull }} has developed a series of translators. For more information, see {% if book.targetDVProd %}xref:translators{% else %}link:as_translators.adoc{% endif %}[Translators]. 
{% if book.targetWildfly %}
