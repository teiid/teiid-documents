...
extension metadata property to the appropriate source tables. Extension properties can be set at design time in Designer, in the vdb.xml when using full link:DDL_Metadata.adoc[DDL Metadata] or alter statements, or at runtime using the setProperty link:System_Schema.adoc[system procedure]. The property is expected to be a comma separated list of physical model/schema names.
