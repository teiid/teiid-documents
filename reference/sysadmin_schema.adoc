...
Assuming materialized view `SAMPLEMATVIEW` has 3 rows under the `TestInterMat` Model, the primary key only have one column, id is the primary key:
...
If the primary key only comtain one column, id is the primary key, update the second row:

[source,sql]
----
EXEC SYSADMIN.refreshMatViewRow('TestInterMat.SAMPLEMATVIEW', '101')
----

If the primary key comtain more columns, id, a and b compose of the primary key, update the second row:

[source,sql]
----
EXEC SYSADMIN.refreshMatViewRow('TestInterMat.SAMPLEMATVIEW', '101', 'a1', 'b1')
----
...
Continuing use the `SAMPLEMATVIEW` in Example of <<SYSADMIN.refreshMatViewRow, SYSADMIN.refreshMatViewRow>>, if the primary key only comtain one column, id is the primary key, update all rows:

[source,sql]
----
EXEC SYSADMIN.refreshMatViewRows('TestInterMat.SAMPLEMATVIEW', ('100',), ('101',), ('102',))
---- 

If the primary key comtain more columns, id, a and b compose of the primary key, update all rows:

[source,sql]
----
EXEC SYSADMIN.refreshMatViewRows('TestInterMat.SAMPLEMATVIEW', ('100', 'a0', 'b0'), ('101', 'a1', 'b1'), ('102', 'a2', 'b2'))
----
