...
The planner logically treats a multisource table as if it were a view containing the union all of the respective source tables.  More complex partitioning scenarios, such as heterogeneous sources or list partitioning will require the use of a link:Federated_Optimizations.adoc#18646293_FederatedOptimizations-PartitionedUnion[Federated Optimizations#Partitioned Union].
