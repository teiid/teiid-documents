...
A view defined by a `UNION ALL` can support inherent `INSERTs` if it is a partitioned union, and the `INSERT` specifies values that belong to a single partition.
