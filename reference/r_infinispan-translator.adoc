...
- When using DATE/TIMESTAMP/TIME types in {{ book.productnameFull }} metadata, they are by default marshaled into a LONG type in Infinispan.  This means that TIMESTAMPs only retains millisecond precision.
