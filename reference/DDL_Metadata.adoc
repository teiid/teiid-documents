...
WARNING: Not using a semicolon delimiter between statements is deprecated and should only be relied on for backwards compatibility.
...
|UPDATECOUNT
|int
|if this procedure updates the underlying sources, how many updates may be issued. When update count is >1 the XA protocol for execution is enforced. Defaults to -1 AUTO.
|===
