...
|<<createDDLProcedure,create procedure>>, <<createDataWrapper,create data wrapper>>, <<createDatabase,create database>>, <<createDomain,create a domain or type alias>>, <<createForeignTempTable,create foreign temp table>>, <<createPolicy,CREATE POLICY>>, <<createRole,create role >>, <<createSchema,create schema>>, <<createServer,create server>>, <<createTable,create table>>, <<createTempTable,create temporary table>>, <<createTrigger,create trigger>>, <<procedureBodyCommand,procedure body definition>>
...
|<<alter,alter>>, <<alterProcedure,ALTER PROCEDURE >>, <<createDDLProcedure,create procedure>>, <<createPolicy,CREATE POLICY>>, <<dropPolicy,DROP POLICY>>, <<dropProcedure,drop procedure>>, <<grant,GRANT>>, <<procedureBodyCommand,procedure body definition>>, <<revokeGrant,Revoke GRANT>>
