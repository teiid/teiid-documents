...
A Translator is the heart of the TCA and acts as the bridge logic between {{ book.productnameFull }} and an external system, which is most commonly accessed through a JCA resource adapter. Refer to the {{ book.productnameFull }} Developers Guide for details on developing custom Translators and JCA resource adapters for use with {{ book.productnameFull }}.
