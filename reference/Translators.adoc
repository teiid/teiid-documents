...
TIP: The import capabilities of Teiid Translators is currently only used in link:Dynamic_VDBs.adoc[Dynamic VDBs] and not by Teiid Designer.
...
NOTE: Only a subset of the supports metadata can be set through execution properties. If more control is needed, please consult the https://docs.jboss.org/author/display/TEIID/Developer%27s+Guide[Developer’s Guide].
...
See also link:VDB_Definition.adoc[VDB Definition].
