...
[1] JavaDoc for http://java.sun.com/javase/6/docs/api/java/sql/DatabaseMetaData.html[DatabaseMetaData] +
...
Example importer settings to only import tables and views from my-schema. See also link:VDB_Definition.html[VDB Definition]
...
By default the name of the procedure that executes the queries directly is *native*. link:Translators.adoc#18646290_Translators-OverrideExecutionProperties[Override the execution property] _DirectQueryProcedureName_ to change it to another name.
...
The resource adapter for this translator provided through data source in JBoss AS, See to Admin Guide section https://docs.jboss.org/author/display/TEIID/JDBC+Data+Sources[JDBC Data Sources] for configuration.
