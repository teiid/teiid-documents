...
{{ book.productnameFull }} supports two forms of the CASE expression which allows conditional logic in a scalar expression.
...
{{ book.productnameFull }} parses and evaluates operators with higher precedence before those with lower precedence. Operator with equal precedence are left associative. Operator precedence listed from high to low:
