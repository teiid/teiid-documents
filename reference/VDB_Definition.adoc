...
Set to a list of modules for the vdb classpath for user defined function loading. See also https://docs.jboss.org/author/display/TEIID/Support+for+User-Defined+Functions+%28Non-Pushdown%29[Support for User-Defined Functions (Non-Pushdown)].
...
For use by the ODBC transport and OData to set default connection/execution properties. See https://docs.jboss.org/author/display/TEIID/Driver+Connection[Driver Connection] for all properties. Note these are set on the connection after it has been established.
...
By default, PG metadata is always added to VDB unless https://docs.jboss.org/author/display/TEIID/System+Properties[System Properties] set property _org.teiid.addPGMetadata_ to false. This property enables adding PG metadata per VDB. Please note that if you are using ODBC to access your VDB, the VDB must include PG metadata.

* _lazy-invalidate_ 

By default TTL expiration will be invalidating - see https://docs.jboss.org/author/display/TEIID/Internal+Materialization[Internal Materialization]. Setting lazy-invalidate to true will make ttl refreshes non-invalidating.
