...
...
GRANT SELECT ON TABLE test.Orders CONDITION 'amount < 1000' TO base-role;
----

.Example : Override previous example to more privileged user.
[source,sql]
----
GRANT SELECT ON TABLE test.Orders CONDITION 'amount < 1000 and amount >=1000' TO RoleA;
----

.Example : Restricting rows to only those owned by this user.
[source,sql]
----
GRANT SELECT ON TABLE test.CustomerOrders CONDITION CONSTRAINT 'name = user()' TO RoleA;
----
