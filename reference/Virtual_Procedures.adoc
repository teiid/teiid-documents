...
In DDL: link:DDL_Metadata.adoc#20316193_DDLMetadata-CreateProcedure%2FFunction[DDL Metadata#Create Procedure/Function]
...
You execute procedures using the SQL link:DML_Commands.adoc#18646216_DMLCommands-EXECUTECommand[EXECUTE] command. If the procedure has defined inputs, you specify those in a sequential list, or using "name=value" syntax. You must use the name of the input parameter, scoped by the full procedure name if the parameter name is ambiguous in the context of other columns or variables in the procedure.
