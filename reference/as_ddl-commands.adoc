...
For information about the DDL statements that you can use to define schemas in a virtual database, see {% if book.targetDVProd %}xref:ddl-metadata-for-schema-objects{% else %}link:r_ddl-metadata-for-schema-objects.adoc{% endif %}[DDL metadata].
