...
This document will provide detailed instructions to enable OAuth V2 authentication on {{ book.productnameFull }}'s OData interface using the Keycloak as authentication server (IDP). Please note that use a different IDP server will not work with this implementation as OAuth2 implementations are not interoperable. To work with separate IDP than Keycloak consult their documentation, replace the web layer semantics, like the "login-config" in web.xml file etc. Providing the details of other IDP is beyond the scope of this document.

This examples will show case an example, where {{ book.productnameFull }}'s OData rest interface is secured using OAuth using Keycloak using OpenID Connect. The VDB accessed by the OData interface also depends on another web service which is used as a data source, that is also secured with OAuth using the same Keycloak IDP. The central idea behind this example is to pass the same "access-token" used at OData interface layer to passthrough the {{ book.productnameFull }} layer to bottom data source layer and gain access to the source.
 ...
 * Add a new client called "odata4-oauth", this client represents the {{ book.productnameFull }}'s OData client that we are going to create
...
* Download Keycloak adapter for the EAP, and unzip over the {{ book.productnameFull }} server installation or follow Keycloak installtion directions.
...
Now, start the {{ book.productnameFull }} Server and using the jboss-cli.sh file run the following to install the KeyCloak configuration into the {{ book.productnameFull }} Server.
...
In order to use OAuth2 authentication, the OData WAR needs to be updated to make use of the OAuth2 based security domain. By default {{ book.productnameFull }} installation comes with OData web service WAR file configured with "HTTP Basic" authentication. This WAR needs to either replaced or updated.

=== Build the new OData WAR file that supports OAuth.

To build OAuth based OData WAR file, {{ book.productnameFull }} provides a template maven project, either download or clone the project from https://github.com/teiid/teiid-web-security 

The above link provides templates for creating two WAR files, one WAR file is to create {{ book.productnameFull }}'s OData service with OAuth, the next is a sample "database-service" for this demo. Please note that "database-service" is to mimic the database service, that will be different in a real use-case, however the steps defined for the access will be same.
...
==== {{ book.productnameFull }} Server on WildFly 
...
Start both Keycloak and {{ book.productnameFull }} Servers. If both of these servers are in the same machine, then we need to offset the ports of {{ book.productnameFull }} server such that they will not conflict with that of the Keycloak server. For this example, I started the {{ book.productnameFull }} server as
...
There are two different mechanisms for testing this example. One is purely for testing the using the browser, then other is programatically. Typically using the browser is NOT correct for accessing the {{ book.productnameFull }}'s OData service, but it is shown below for testing purposes.
...
This process of calling does not need to involve a web-browser, this is typical of scenario where another web-application or mobile application is calling the {{ book.productnameFull }}'s OData web-service to retrieve the data. However in this process, the process of negotiating the "access-token" is externalized and is defined by the IDP, which in this case is Keycloak.
