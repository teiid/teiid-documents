...
NOTE: With default settings and an 8GB VM size[\*], then _max-reserve-kb_ will at a max use: (((1024-300) \* 0.4) + (7 \* 1024 \* 0.5)) = 4373.6 MB or 4,478,566 KB - before considering the overhead for persistent batches or the fixed memory buffer.  The fixed memory buffer will by default use 40% of that initial calculation.  Once additional overhead is removed, the actual max reserve kb will be around 2.3 GB.
