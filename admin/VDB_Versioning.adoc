...
When a user connects to Teiid the desired VDB version can be set as a connection property (See the https://docs.jboss.org/author/display/TEIID/Client+Developer%27s+Guide[Client Developer’s Guide]) in JDBC or used as part of the VDB name for OData and ODBC access.
