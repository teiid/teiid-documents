...
JDBC clients may need to adjust low-level transport values, in addition to https://docs.jboss.org/author/display/TEIID/SSL+Client+Connections[SSL Client Connection] properties via a teiid-client-settings.properties file. This file also contains buffer, socket pooling, and maxObjectSize (effectively the maximum response size) settings.
