...
|Set to true to have XPath PATH values beginning with / and // in XMLTABLE always be relative to the context item (the same behavior as Oracle).  Set to false to have the path value to be evaluated from the root of the context item (the same behavior as PostgreSQL).
|true
