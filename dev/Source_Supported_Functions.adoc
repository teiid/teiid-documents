...
If you source handing of the function can be described by simple parameter substitution into a string, then you may not need to extend the translator for a source specific function. You can use the extension property teiid_rel:native-query to define the syntax handling - see also https://docs.jboss.org/author/display/TEIID/DDL+Metadata[DDL Metadata] for functions.
...
== When Using Dynamic VDB

If you are using the Dynamic VDB, and defining the metadata https://docs.jboss.org/author/display/TEIID/DDL+Metadata[using DDL], you can define your source function in the VDB as
