...
The JDG can be used as  materialization target to save cache contents of the View in  JDG server, which can be cluster aware, that will be accessed using the JDG Hot Rod Client.  See the link:../admin/Infinispan_HotRod_Data_Sources.adoc[JDG Hot Rod DataSource] for how to configure accessing the remote cache. 
