...
Users when they are designing their views, they can define additional metadata on their views to control the loading and refreshing of external materialization cache. This option provides a limited but a powerful way to manage the materialization views. For this purpose, link:../reference/sysadmin_schemas.adoc#_virtual_procedures[SYSADMIN Schema] model in your VDB defines three stored procedures (link:../reference/sysadmin_schemas.adoc#_sysadmin_loadmatview[loadMatView], link:../reference/sysadmin_schemas.adoc#_sysadmin_updatematview[updateMatView], link:../reference/sysadmin_schemas.adoc#_sysadmin_matviewstatus[matViewStatus]) in its schema. Based on the defined metadata on the view, and these Procedures use a simple scheduler automatically starts during the VDB deployment and loads and keeps the cache fresh.
